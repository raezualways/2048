...
function toggleButtons(show) { localStorage.showButtons = show ? '1' : '0'; if(document.getElementById('btnToggle')) document.getElementById('btnToggle').checked = show; }
...
function renderAchievementsTab() {
    ...
    for (const id in achievements) {
        ...
        card.className = `achievement ${a.earned ? 'earned' : ''}`;
        card.innerHTML = `
...
function switchTab(tabIndex) {
    ...
    const gameSection = document.querySelector('.sheet-content > div:nth-child(2)');
    const duelsSection = document.querySelector('.sheet-content > div:nth-child(3)');
    const themesSection = document.querySelector('.sheet-content > div:nth-child(4)');
    const statsSection = document.querySelector('.sheet-content > div:nth-child(5)');

    // Hide all sections
    if (gameSection) gameSection.style.display = 'none';
    if (duelsSection) duelsSection.style.display = 'none';
    ...
    navItems.forEach((item, index) => {
        if (index === tabIndex) {
            item.style.background = 'var(--empty)';
        } else {
...
// Initialize settings menu with first tab active
document.addEventListener('DOMContentLoaded', function() {
    // Show only the first tab content initially
    const contentSections = document.querySelectorAll('.sheet-content > div:not(.bottom-nav)');
    ...
    const firstNavItem = document.querySelector('.bottom-nav div:first-child');
    if (firstNavItem) {
        firstNavItem.style.background = 'var(--empty)';
    }
});
